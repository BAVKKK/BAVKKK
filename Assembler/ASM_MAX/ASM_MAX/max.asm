...
.DATA							 ; Директива определяет начало сегмента данных (данные, которые имеют начальное значение)
	STR_F DB "Введите первое число, а затем второе в восьмеричной системе: ", 10, 0
	...
	STR_ERR DB "Ошибка. Некорректный ввод. Проверьте длину вводимого числа и допустимые данной сс символы", 0	
...
.CODE							; Директива определяет начало сегмента кода
	...
	NEXT:	
	CMP BL,'7'
	JBE NEXT1				; если символ <='7' то обрабатываем
	JMP ERROR
	NEXT1:
	...
	NEXT_:	
	CMP BL,'7'
	JBE NEXT_1				; если символ <='7' то обрабатываем
	JMP ERROR
	NEXT_1:
	SUB BL, '0'				; вычитаем '0' чтобы получить число
	MOV EDX, 8 
	...
	MOV EAX, FIRST
	MOV EBX, SECOND
	CMP F_SIGN, 0 ; Если знак 0 продолжаем
	JNE FS_1
	CMP S_SIGN, 0 ; Если не 0 прыгаем в условие +-
	JNE FSS_01
	SUB FIRST, EBX ; Случай с ++
	JMP RES
